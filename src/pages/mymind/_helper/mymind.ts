...
MM.Item.prototype._updateValue = function() {
  ...
  this._dom.value.innerHTML =
    Math.round(result) == result ? result : result.toFixed(3);
};
...
MM.Map.prototype._getPickCandidates = function(
  currentRect,
  item,
  direction,
  candidates,
...
MM.Command.Save = Object.create(MM.Command, {
  ...
  keys: {
    value: [{ keyCode: 'S'.charCodeAt(0), ctrlKey: true, shiftKey: false }],
  },
});
...
MM.Command.SaveAs = Object.create(MM.Command, {
  ...
  keys: {
    value: [{ keyCode: 'S'.charCodeAt(0), ctrlKey: true, shiftKey: true }],
  },
});
...
MM.Command.Finish = Object.create(MM.Command, {
  keys: {
    value: [{ keyCode: 13, altKey: false, ctrlKey: false, shiftKey: false }],
  },
  editMode: { value: true },
});
...
MM.Command.Value = Object.create(MM.Command, {
  ...
  keys: {
    value: [{ charCode: 'v'.charCodeAt(0), ctrlKey: false, metaKey: false }],
  },
});
...
MM.Command.Computed = Object.create(MM.Command, {
  ...
  keys: {
    value: [{ charCode: 'c'.charCodeAt(0), ctrlKey: false, metaKey: false }],
  },
});
...
MM.Layout.Tree._layoutChildren = function(
  children,
  rankDirection,
  offset,
  bbox,
...
MM.Layout.Tree._drawLines = function(item, side) {
  ...
  var y2 =
    last.getShape().getVerticalAnchor(last) + last.getDOM().node.offsetTop;
...
MM.Layout.Map._layoutRoot = function(item) {
  ...
  dom.content.style.top =
    Math.round((height - dom.content.offsetHeight) / 2) + 'px';
...
MM.Format.FreeMind._serializeAttributes = function(doc, json) {
  ...
  if (json.notes) {
    ...
    notesElm.appendChild(
      doc.createCDATASection(
        '<html><head></head><body>' + json.notes + '</body></html>',
      ),
    );
...
MM.Backend.GDrive = Object.create(MM.Backend, {
  ...
  clientId: {
    value:
      '767837575056-h87qmlhmhb3djhaaqta5gv2v3koa9hii.apps.googleusercontent.com',
  },
...
MM.UI.Status.prototype.handleEvent = function(e) {
  var action = new MM.Action.SetStatus(
    MM.App.current,
...
MM.UI.Icon.prototype.handleEvent = function(e) {
  var action = new MM.Action.SetIcon(
    MM.App.current,
    this._select.value || null,
  );
  MM.App.action(action);
...
MM.UI.Backend.File.init = function(select) {
  ...
  this._format.value =
    localStorage.getItem(this._prefix + 'format') || MM.Format.JSON.id;
};
...
MM.UI.Backend.File.save = function() {
  ...
  this._backend
    .save(data, name)
    .then(this._saveDone.bind(this), this._error.bind(this));
};
...
MM.UI.Backend.WebDAV._load = function(url) {
  ...
  this._backend
    .load(url)
    .then(this._loadDone.bind(this), this._error.bind(this));
};
...
MM.UI.Backend.Firebase.init = function(select) {
  ...
  this._server.value =
    localStorage.getItem(this._prefix + 'server') || 'my-mind';
...
MM.UI.Backend.Firebase.save = function() {
  ...
  var map = MM.App.map;
  this._backend
    .save(map.toJSON(), map.getId(), map.getName())
    .then(this._saveDone.bind(this), this._error.bind(this));
};
...
MM.UI.Backend.Firebase._sync = function() {
  ...
  this._go.innerHTML =
    this._mode.charAt(0).toUpperCase() + this._mode.substring(1);
};
...
MM.UI.Backend.GDrive._loadDone = function(data) {
  try {
    var format =
      MM.Format.getByMime(data.mime) ||
      MM.Format.getByName(data.name) ||
      MM.Format.JSON;
    var json = format.from(data.data);
  } catch (e) {
    this._error(e);
...
MM.Mouse._finishDragDrop = function(state) {
  ...
  switch (state.result) {
    ...
    case 'sibling':
      ...
      var action = new MM.Action.MoveItem(
        this._item,
        target.getParent(),
        targetIndex,
        target.getSide(),
...
MM.Mouse._computeDragState = function() {
  ...
  var closest = MM.App.map.getClosestItem(
  ...
  if (target.isRoot()) {
    ...
    var childDirection = target
      .getParent()
      .getLayout()
      .getChildDirection(target);
    var diff =
      -1 *
      (childDirection == 'top' || childDirection == 'bottom'
        ? closest.dx
        : closest.dy);
...
MM.Mouse._visualizeDragState = function(state) {
  if (
    this._oldState &&
    state &&
    this._oldState.item == state.item &&
    this._oldState.result == state.result
  ) {
    return;
  } /* nothing changed */
  ...
  if (state) {
    ...
    var spread = x || y ? -2 : 2;
    node.style.boxShadow =
      x * offset + 'px ' + y * offset + 'px 2px ' + spread + 'px #000';
  }
};
/*
